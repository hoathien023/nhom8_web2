<?php
    class ProductModel {
        ...
        public function select_products_by_cate_paginated($category_id, $page = 1, $perPage = 9) {
            $start = max(0, ((int)$page - 1) * (int)$perPage);
            $sql = "SELECT * FROM products WHERE category_id = ? AND status = 1 ORDER BY product_id DESC LIMIT " . $start . ", " . (int)$perPage;
            return pdo_query($sql, (int)$category_id);
        }

        public function count_products_by_cate($category_id) {
            $sql = "SELECT COUNT(*) AS total FROM products WHERE category_id = ? AND status = 1";
            return pdo_query_one($sql, (int)$category_id);
        }
